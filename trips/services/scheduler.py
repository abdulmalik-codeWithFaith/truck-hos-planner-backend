...
required 10-hour rests — respecting the 11-hour/14-hour/70-hour limits at
every step. This is the single source of truth the ELD generator and the
API response both consume; nothing downstream recomputes HOS logic.
        ...
        while remaining_leg_miles > FLOAT_TOLERANCE:
            ...
            if hos.cycle_exhausted(state):
                ...
                events.append(ScheduleEvent(
                    ...
                    duration_hours=hos.RESTART_HOURS,
                    status="OFF_DUTY",
                    label="34-hour restart (resets 70-hour/8-day cycle)",
                    location=f"Rest stop (~{round(total_miles_driven)} mi)",
                ))
                continue
            ...
            if available_hours <= FLOAT_TOLERANCE:
                ...
                fraction = 1 - (remaining_leg_miles / leg.distance_miles) if leg.distance_miles else 0
                lat, lng = _interpolate(leg_start_wp, leg_end_wp, fraction)
                state = hos.advance_rest(state, hos.REQUIRED_REST_HOURS)
                events.append(ScheduleEvent(
                    day=day_indexer.day_for(seg_start),
                    start_time=seg_start,
                    end_time=state.clock,
                    duration_hours=hos.REQUIRED_REST_HOURS,
            ...
            if miles_until_fuel <= FUEL_MILE_TOLERANCE:
                hours_until_fuel = 0.0  # threshold reached or passed — fuel now
